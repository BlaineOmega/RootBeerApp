...
-(id) initViewWithID:(NSString*)ID
{
    if((self = [super init]))
    {
        ...
        else if([ID isEqualToString:ROOTBEERS])
        {
            mViewController = [[RootBeerTableViewController alloc] initWithNibName:ID bundle:nil];
        }
        else if([ID isEqualToString:DETAILVIEW])
        {
            mViewController = [[DetailViewController alloc] initWithNibName:ID bundle:nil];
        }else if([ID isEqualToString:RTVC])
        {
            mViewController = [[RootBeerTVCViewController alloc] initWithNibName:ID bundle:nil];
        }
...
-(View*) getCurrentView
{
    return [mViewStack lastObject];
}
-(id) getCurrentViewController
{
    View* currentView = [self getCurrentView];
    return currentView.mViewController;
}
-(id) init
{
    if((self = [super init]))
...
-(void) pushView:(NSString*)ID animated:(BOOL)anim
{
    ...
    if(newView == nil)
    {
        newView = [[View alloc] initViewWithID:ID];
...
{
    View* view = [mViewStack lastObject];
    if(view.mCache == YES)
    {
    ...
    if(VIEW_DEBUG)
    {
        ...
        {
            View* currentView = [mViewStack objectAtIndex:mViewStack.count-1];
...
{
    View* desiredController;
    for(View* v in mViewStack)
    {
        if(v == view)
        {
            desiredController = v;
